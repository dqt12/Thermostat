...
/* SD clock freq                                                                                            */
#define SDIO_INIT_CLK_DIV               ((u32)(SystemCoreClock/400000))
#define SDIO_HIGH_SPEED_CLK_DIV         ((u32)3)
#define SDIO_NORMAL_SPEED_CLK_DIV       ((u32)3)
...
static SD_Error GetCardInfo(void);
static SD_Error FindSCR(void);
...
bool SD_Detect(void)
{
  ...
  /* SD card detection */
  if (GPIO_ReadInBit(SDCARD_SD_CD_GPIO_PORT, SDCARD_SD_CD_GPIO_PIN))
  {
    status = FALSE;
  }
...
/*********************************************************************************************************//**
  ...
  * @retval None
  ***********************************************************************************************************/
void SD_LowLevel_DMA_RxConfig(u8 *BufferDST, u32 BufferSize)
{
...
/*********************************************************************************************************//**
  ...
  * @retval None
  ***********************************************************************************************************/
void SD_LowLevel_DMA_RxConfig_fixDST(u8 *BufferDST, u32 BufferSize)
{
  PDMACH_InitTypeDef PDMACH_InitStruct;
  ...
  PDMACH_InitStruct.PDMACH_SrcAddr = (u32)&HT_SDIO->DR;
  PDMACH_InitStruct.PDMACH_DstAddr = (u32)BufferDST;
  PDMACH_InitStruct.PDMACH_AdrMod = SRC_ADR_FIX | SRC_ADR_FIX;
  PDMACH_InitStruct.PDMACH_BlkCnt = BufferSize / 16;
  PDMACH_InitStruct.PDMACH_BlkLen = 4;
  PDMACH_InitStruct.PDMACH_DataSize = WIDTH_32BIT;
  PDMACH_InitStruct.PDMACH_Priority = H_PRIO;
...
/*********************************************************************************************************//**
  ...
  * @retval None
  ***********************************************************************************************************/
void SD_LowLevel_DMA_TxConfig(u8 *BufferSRC, u32 BufferSize)
{
  PDMACH_InitTypeDef PDMACH_InitStruct;
...
SD_Error SD_Init(void)
{
  ...
  GPIO_DirectionConfig(SDCARD_SD_CD_GPIO_PORT, SDCARD_SD_CD_GPIO_PIN, GPIO_DIR_IN);

  /* Configure SDIO pins                                                                                    */
  AFIO_GPxConfig(SDCARD_SD_CLK_GPIO_ID, SDCARD_SD_CLK_AFIO_PIN, SDCARD_SD_CLK_AFIO_MODE);
  AFIO_GPxConfig(SDCARD_SD_CMD_GPIO_ID, SDCARD_SD_CMD_AFIO_PIN, SDCARD_SD_CMD_AFIO_MODE);
  AFIO_GPxConfig(SDCARD_SD_DAT0_GPIO_ID, SDCARD_SD_DAT0_AFIO_PIN, SDCARD_SD_DAT0_AFIO_MODE);
  AFIO_GPxConfig(SDCARD_SD_DAT1_GPIO_ID, SDCARD_SD_DAT1_AFIO_PIN, SDCARD_SD_DAT1_AFIO_MODE);
  AFIO_GPxConfig(SDCARD_SD_DAT2_GPIO_ID, SDCARD_SD_DAT2_AFIO_PIN, SDCARD_SD_DAT2_AFIO_MODE);
  AFIO_GPxConfig(SDCARD_SD_DAT3_GPIO_ID, SDCARD_SD_DAT3_AFIO_PIN, SDCARD_SD_DAT3_AFIO_MODE);

  GPIO_DriveConfig( SDCARD_SD_CLK_GPIO_PORT, SDCARD_SD_CLK_AFIO_PIN, GPIO_DV_8MA);
  GPIO_DriveConfig( SDCARD_SD_CMD_GPIO_PORT, SDCARD_SD_CMD_AFIO_PIN, GPIO_DV_8MA);
  GPIO_DriveConfig(SDCARD_SD_DAT0_GPIO_PORT, SDCARD_SD_DAT0_AFIO_PIN, GPIO_DV_8MA);
  GPIO_DriveConfig(SDCARD_SD_DAT1_GPIO_PORT, SDCARD_SD_DAT1_AFIO_PIN, GPIO_DV_8MA);
  GPIO_DriveConfig(SDCARD_SD_DAT2_GPIO_PORT, SDCARD_SD_DAT2_AFIO_PIN, GPIO_DV_8MA);
  GPIO_DriveConfig(SDCARD_SD_DAT3_GPIO_PORT, SDCARD_SD_DAT3_AFIO_PIN, GPIO_DV_8MA);
  ...
  /* Select card */
  errorstatus = SD_SelectDeselect(SDCardInfo.RCA << 16);
  ...
  errorstatus = SD_BusModeConfig(SD_4BIT_BUS_MODE);

  if (errorstatus != SD_OK)
  {
    return(errorstatus);
  }
  printf("Enable 4-bit bus mode\r\n");

//  errorstatus = SD_GetSDStatus(cardinfo);
  return(SD_OK);
}
...
{
  SD_Error errorstatus;
  u32 SDType, count = 0, response = 0;
  bool validvoltage = FALSE;

  /* Configure SD_CLK to 400 kHz and 1-bit normal speed mode */
...
SD_Error SD_InitializeCards(void)
{
  ...
  if ((SDCardInfo.CardType == SDIO_STD_CAPACITY_SD_CARD_V1_1) ||
      (SDCardInfo.CardType == SDIO_STD_CAPACITY_SD_CARD_V2_0) ||
      (SDCardInfo.CardType == SDIO_HIGH_CAPACITY_SD_CARD)     ||
      (SDCardInfo.CardType == SDIO_SECURE_DIGITAL_IO_COMBO_CARD))
  {
    /* CMD3 */
    SDIO_CmdInitStructure.SDIO_Argument   = 0x0;
    ...
    SDIO_CmdInitStructure.SDIO_CmdCrcChk  = SDIO_CmdCrcChk_Yes;
    SDIO_SendCommand(&SDIO_CmdInitStructure);
    errorstatus = CmdResp6Error(&SDCardInfo.RCA);
  ...
  {
    /* CMD9 */
    SDIO_CmdInitStructure.SDIO_Argument   = SDCardInfo.RCA << 16;
  ...
  errorstatus = GetCardInfo();
//  printf("\nCardType : %u", SDCardInfo.CardType);
//  printf("\nCapacity : %u MB", SDCardInfo.CardCapacity/(1024*1024));
//  printf("\nBlockSize: %u", SDCardInfo.CardBlockSize);
//  printf("\nRCA      : 0x%04x", SDCardInfo.RCA);
//  printf("\nManufID  : %u", SDCardInfo.SD_cid.ManufactID);
...
/*********************************************************************************************************//**
  ...
  * @return The SD card error code
  ***********************************************************************************************************/
SD_Error SD_BusModeConfig(u32 BusMode)
{
  SD_Error errorstatus;
...
SD_Error SD_GetSDStatus(void)
{
  ...
  SDCardInfo.CardStatus.DataBusWidth = (tmp & 0xC0) >> 6;
  SDCardInfo.CardStatus.SecuredMode = (tmp & 0x20) >> 5;

  /* Byte 2 */
  tmp = (u8)((sdstatus[0] & 0x0000FF00) >> 8);
  SDCardInfo.CardStatus.SDCardType = tmp << 8;

  /* Byte 3 */
  tmp = (u8)(sdstatus[0] & 0x000000FF);
  SDCardInfo.CardStatus.SDCardType |= (tmp);

  /* Byte 4 */
  tmp = (u8)((sdstatus[1] & 0xFF000000) >> 24);
  SDCardInfo.CardStatus.ProtectedAreaSize = tmp << 24;

  /* Byte 5 */
  tmp = (u8)((sdstatus[1] & 0x00FF0000) >> 16);
  SDCardInfo.CardStatus.ProtectedAreaSize |= (tmp << 16);

  /* Byte 6 */
  tmp = (u8)((sdstatus[1] & 0x0000FF00) >> 8);
  SDCardInfo.CardStatus.ProtectedAreaSize |= (tmp << 8);

  /* Byte 7 */
  tmp = (u8)(sdstatus[1] & 0x000000FF);
  SDCardInfo.CardStatus.ProtectedAreaSize |= (tmp);

  /* Byte 8 */
  tmp = (u8)((sdstatus[2] & 0xFF000000) >> 24);
  SDCardInfo.CardStatus.SpeedClass = tmp;

  /* Byte 9 */
  tmp = (u8)((sdstatus[2] & 0x00FF0000) >> 16);
  SDCardInfo.CardStatus.PerformanceMove = tmp;

  /* Byte 10 */
  tmp = (u8)((sdstatus[2] & 0x0000FF00) >> 8);
  SDCardInfo.CardStatus.AUSize = (tmp & 0xF0) >> 4;

  /* Byte 11 */
  tmp = (u8)(sdstatus[2] & 0x000000FF);
  SDCardInfo.CardStatus.EraseSize = tmp << 8;

  /* Byte 12 */
  tmp = (u8)((sdstatus[3] & 0xFF000000) >> 24);
  SDCardInfo.CardStatus.EraseSize |= (tmp);

  /* Byte 13 */
  tmp = (u8)((sdstatus[3] & 0x00FF0000) >> 16);
  SDCardInfo.CardStatus.EraseTimeout = (tmp & 0xFC) >> 2;
  SDCardInfo.CardStatus.EraseOffset = (tmp & 0x3);
...
/*********************************************************************************************************//**
  ...
  * @return The SD card error code
  ***********************************************************************************************************/
SD_Error SD_ReadBlock(u8 *readbuff, u32 ReadAddr, u32 BlockSize)
{
  SD_Error errorstatus;
...
/*********************************************************************************************************//**
  ...
  * @return The SD card error code
  ***********************************************************************************************************/
//SD_Error SD_ReadMultipleBlock(u32 ReadAddr, u32 *readbuff, u16 BlockSize, u16 BlockCount)
SD_Error SD_ReadMultipleBlock(u8* pBuf, u32 uReadAddr, u32 uReadSize, u8 mode)
{
  SD_Error errorstatus;
	u32 BlockSize = 512;
  u32 BlockCount = (uReadSize+511)/512;
  ...
  if(mode)
    SD_LowLevel_DMA_RxConfig_fixDST(pBuf, uReadSize);
  else
    SD_LowLevel_DMA_RxConfig(pBuf, uReadSize);

  /* CMD18 */
  SDIO_CmdInitStructure.SDIO_Argument   = uReadAddr/512;
...
void check_SD_ReadMultipleBlock_end(void)
{
  while ((TransferEnd == 0) && (TransferStatus == SD_OK))
  {
  }
...
SD_Error SD_Erase(u32 StartAddr, u32 EndAddr)
{
  ...
  maxdelay = (SystemCoreClock / ((HT_SDIO->CLKCR >> 8) + 1)) * 0.005;  // 5 ms
...
/*********************************************************************************************************//**
  ...
  * @return The SD card error code
  ***********************************************************************************************************/
SD_Error SD_WriteMultiBlocks(u8 *writebuff, u32 WriteAddr, u16 BlockSize, u16 BlockCount)
{
  SD_Error errorstatus;
  u8 cardstate;
  u32 timeout=1000;
...
static SD_Error FindSCR(void)
{
  ...
  SDCardInfo.CardBlankValue = (((SCR[0] >> 23) & 1UL) == 0) ? (0) : (0xFFFFFFFF);
